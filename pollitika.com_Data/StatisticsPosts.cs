...
namespace pollitika.com_Data
{
    public class StatisticsPosts
    {
        ...
        public static void GetPostsWithMostNegativeVotes(int numPosts, ModelRepository inRepo)
        {
            ...
            foreach (var post in list)
                Console.WriteLine("Post by {0,-18}, negative votes {1}, post - {2}", post.Author.NameHtml, post.GetNumberOfNegativeVotes(), post.Title);
            Console.WriteLine("");
        }
        public static void GetMostControversialPosts(int numPosts, ModelRepository inRepo)
        {
            ...
            foreach (var post in list)
                Console.WriteLine("Post by {0,-18}, num.votes {1}, neg.votes {2}, post - {3}", post.Author.NameHtml, post.GetNumberOfVotes(), post.GetNumberOfNegativeVotes(), post.Title);
            Console.WriteLine("");
        }
        public static void GetMostCommentedPosts(int numPosts, ModelRepository inRepo)
        {
            ...
            foreach (var post in list)
                Console.WriteLine("Post by {0,-18}, num.comments {1}, post - {2}", post.Author.NameHtml, post.GetNumberOfComments(), post.Title);
            Console.WriteLine("");
        }
